...
const ShareModal = ({pubChoosen, publicationId, creatorName, creatorId, closeModalShare, profileImage, }) => { 
   ...
   const userCtx = useContext(UserContext)
    ...
    useEffect(() => { 
      axios.get(`http://localhost:4000/getOnePublication/${publicationId}`)
          .then((res) => { 
    ...
    function openModalFour() {
      ...
      const shareOnMyWall = () => {
          const publicationDataToBeShared = ({ 
            ...
            publicationImages: [pubChoosen.publicationImages[0], pubChoosen.publicationImages[1]],
                                            ...
                                            <dialog id="my_modal_4" className="modal ">
                                                   {loading ?  
                                                      <form method="dialog" className="modal-box w-80">
                                                          <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" onClick={() =>closeModalShare()}>✕</button>
                                                           
                                                           <div> 
                                                              <div className='mb-4'>
                                                                <textarea className='rounded-lg w-100% h-auto text-sm' placeholder='Write your comment' onChange={(e) => setCommentShared(e.target.value)}/>
                                                              </div>
                                                                {publicationTotal.map((p) => ( 
                                                                    <div>
                                                                      <div className='flex flex-start'>
                                                                        <img className='rounded-2xl h-8 w-8' src={p.creatorProfileImage}></img>
                                                                      ...
                                                                      <div className='justify-center'>
                                                                          ...
                                                                          <small className='mt-6'>{p.creatorLocation}, {p.address}</small>
                                                                      </div>
                                                                      <div className='flex justify-center mt-6'>
                                                                        <img className=' h-24 w-24 ml-2' src={p.publicationImages[0]}></img>
                                                                        <img className=' h-24 w-24 ml-2' src={p.publicationImages[1]}></img>
                                                                      </div>
                                                                    </div>
                                                                ))}
                                                           </div>
                                                          ...
                                                          <div className=''>
                                                                <button className='bg-blue-950 border-none mt-4 h-9 w-18 text-sm text-white hover:text-black hover:bg-yellow-400' onClick={() => shareOnMyWall()}>
                                                                  Share in my Wall
                                                                  </button>
                                                            </div>
                                                      </form> : 
